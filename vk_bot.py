...
BTN_ABONEMENTS = "🎫 Абонементы"
BTN_UPLOAD_ABONEMENTS = "📤 Обновить абонементы"
BTN_DEPOSIT = "🏦 Депозит"
...
ADMIN_MENU_CMDS = {"new", "lesson", "list", "pay", "link", "delete", "deposit", "card",
                    "upload_schedule", "upload_abonements", "groups", "assign_groups",
                    "report", "dialogs", "restore"}
...
ADMIN_ONLY_EXTRA_CMDS = {
    ...
    "report_run", "noop", "reply_start", "resume_bot", "restore_ok", "restore_cancel",
}
...
STUDENT_MENU = {
    ...
    "buttons": [
        ...
        [_btn(BTN_ABONEMENTS, "abonements")],
        [_btn(BTN_ASK, "ask")],
    ],
}
...
def send_abonements(peer_id: int, user_id: int):
    photo = core.get_setting("abonements_photo")
    kb = ADMIN_MENU if is_admin(user_id) else STUDENT_MENU
    if photo:
        send_with_attachment(peer_id, "🎫 Абонементы:", photo, kb)
    else:
        send(peer_id, "Картинка с абонементами пока не загружена 🙂", kb)


# ------------------------- восстановление учеников из анкет -------------------------
# База может пропасть (например, если файл базы случайно попал в репозиторий и
# затирался при обновлении). Анкеты при этом остаются в переписке педагога —
# по ним восстанавливаем и самих учеников, и привязку их VK-аккаунтов,
# чтобы людям не пришлось регистрироваться заново.

import re as _re

_ANKETA_FIELDS = {
    "child_name": _re.compile(r"^Реб[её]нок:\s*(.+)$", _re.M),
    "adult_name": _re.compile(r"^Взрослый:\s*(.+)$", _re.M),
    "parent_name": _re.compile(r"^Родитель:\s*(.+)$", _re.M),
    "birth_date": _re.compile(r"^Дата рождения:\s*(.+)$", _re.M),
    "phone": _re.compile(r"^Телефон:\s*(.+)$", _re.M),
    "contact_channel": _re.compile(r"^Канал связи:\s*(.+)$", _re.M),
}


def parse_anketas(text: str) -> list[dict]:
    """Разбирает пересланные педагогом сообщения-анкеты. Блоки без имени
    (посторонний текст, случайно попавший в копипасту) пропускаются."""
    chunks = _re.split(r"(?=🆕?\s*Нов(?:ая|ые)\s+анкет)", text)
    if len(chunks) == 1:
        chunks = [text]  # прислали одну анкету без заголовка
    out = []
    for chunk in chunks:
        if not chunk.strip():
            continue
        data = {}
        for key, pat in _ANKETA_FIELDS.items():
            m = pat.search(chunk)
            if m:
...
def fetch_dialog_users() -> dict[int, str]:
    """Все, кто когда-либо писал сообществу: {vk_id: 'Имя Фамилия'}.
    Переписка переживает потерю базы — отсюда и берём привязки."""
    users: dict[int, str] = {}
    offset = 0
    while True:
        resp = api("messages.getConversations", count=200, offset=offset, extended=1)
        if not resp:
            break
        for p in resp.get("profiles", []):
            users[p["id"]] = f"{p.get('first_name','')} {p.get('last_name','')}".strip()
        if len(resp.get("items", [])) < 200:
            break
        offset += 200
...
def match_vk_id(student_name: str, parent_name: str | None,
    ...
    """
    В анкете обычно полное ФИО («Петрова Анна Сергеевна»), а в профиле VK —
    имя и фамилия в другом порядке («Анна Петрова»). Поэтому сравниваем
    наборы слов в обе стороны, требуя минимум два общих слова.
    Для детей пробуем и имя родителя — писал-то обычно он.
    Если подходящих несколько — возвращаем None: лучше не привязать,
    чем привязать чужой аккаунт.
    """
    for cand in (n for n in (student_name, parent_name) if n):
        c_words = set(_norm_name(cand).split())
        if len(c_words) < 2:
            continue
        hits = []
        for uid, name in dialog_users.items():
            n_words = set(_norm_name(name).split())
            if len(n_words) < 2:
                continue
            if len(c_words & n_words) >= 2 and (c_words <= n_words or n_words <= c_words):
                hits.append(uid)
        if len(hits) == 1:
            return hits[0]
    return None
...
def do_restore(anketas: list[dict]) -> tuple[int, int, list[str]]:
    """Создаёт учеников по разобранным анкетам. Возвращает
    (создано, из них с привязкой, список имён без привязки)."""
    dialog_users = fetch_dialog_users()
    created = linked = 0
    unlinked: list[str] = []
    for a in anketas:
        vk_id = match_vk_id(a["name"], a.get("parent_name"), dialog_users)
        if vk_id and not core.get_student_by_user(vk_id, "vk"):
            core.register_with_anketa(
                vk_id, "vk",
                category=a["category"], display_name=a["name"],
                parent_name=a.get("parent_name"), birth_date=a.get("birth_date"),
                phone=a.get("phone"), contact_channel=a.get("contact_channel"),
            )
            linked += 1
        else:
...
def handle_message(msg: dict, client_info: dict | None = None):
    ...
    if cmd == "schedule" or text == BTN_SCHEDULE:
        ...
        return
    if cmd == "abonements" or text == BTN_ABONEMENTS:
        send_abonements(peer_id, user_id)
        return
    ...
    # --- педагог прислал фото абонементов по ранее запрошенному шагу ---
    if st and st.get("step") == "await_abonements_photo" and is_admin(user_id):
        attachment = extract_photo_attachment(msg)
        if attachment:
            core.set_setting("abonements_photo", attachment)
            _state.pop(user_id, None)
            send(peer_id, "Картинка с абонементами успешно обновлена ✅", ADMIN_MENU)
        else:
            send(peer_id, "Пожалуйста, пришлите именно фото — как картинку, не файлом и не ссылкой.")
        return
    ...
    if is_admin(user_id) and text in {
        ...
        BTN_DEPOSIT, BTN_CARD, BTN_UPLOAD_SCHEDULE, BTN_UPLOAD_ABONEMENTS,
        BTN_GROUPS, BTN_ASSIGN_GROUPS, BTN_REPORT, BTN_DIALOGS, BTN_RESTORE,
    }:
...
def _label_to_cmd(label: str) -> str:
    return {
        ...
        BTN_UPLOAD_ABONEMENTS: "upload_abonements",
        BTN_GROUPS: "groups", BTN_ASSIGN_GROUPS: "assign_groups", BTN_REPORT: "report",
        BTN_DIALOGS: "dialogs", BTN_RESTORE: "restore",
    }.get(label, "")
...
def handle_admin_cmd(cmd: str, payload: dict, peer_id: int, user_id: int) -> bool:
    ...
    if cmd == "upload_abonements":
        _state[user_id] = {"step": "await_abonements_photo", "data": {}}
        send(peer_id, "Пожалуйста, пришлите фото с абонементами следующим сообщением (как картинку).",
             CANCEL_MENU)
        return True

    if cmd == "restore":
        _state[user_id] = {"step": "await_anketas", "data": {}}
        send(peer_id,
             "♻️ Восстановление учеников из анкет\n\n"
             "Перешлите (или скопируйте и отправьте) сообщения «🆕 Новая анкета…», "
             "которые бот присылал Вам при регистрации.\n\n"
             "Можно отправить несколько штук одним сообщением, посторонний текст "
             "не помешает. Я покажу, кого нашёл, и спрошу подтверждение.",
             CANCEL_MENU)
        return True

    if cmd == "restore_cancel":
        _state.pop(user_id, None)
        send(peer_id, "Восстановление отменено, в базу ничего не записано.", ADMIN_MENU)
        return True

    if cmd == "restore_ok":
        st = _state.get(user_id) or {}
        anketas = (st.get("data") or {}).get("anketas") or []
        _state.pop(user_id, None)
        if not anketas:
            send(peer_id, "Список анкет потерялся, начните заново.", ADMIN_MENU)
            return True
        send(peer_id, "Восстанавливаю, это займёт несколько секунд…")
        try:
            created, linked, unlinked = do_restore(anketas)
        except Exception:
            log.exception("Ошибка восстановления")
            send(peer_id, "Не удалось восстановить — что-то пошло не так. "
                          "Загляните в логи бота.", ADMIN_MENU)
            return True
        text = (f"✅ Восстановлено учеников: {created}\n"
                f"🔗 Из них с привязкой VK: {linked} — им регистрироваться заново не нужно.")
        if unlinked:
            text += ("\n\n⚠️ Привязку не нашёл у этих (отправьте им ссылку "
                     "кнопкой «🔗 Ссылка»):\n" + "\n".join(unlinked))
        text += ("\n\nОсталось проставить баланс занятий («💰 Оплата») и "
                 "распределить по группам («👥 Группы») — этих данных в анкетах нет.")
...
def handle_dialog(user_id: int, peer_id: int, text: str, st: dict):
    ...
    if step == "await_anketas":
        anketas = parse_anketas(text)
        if not anketas:
            send(peer_id,
                 "Не нашёл ни одной анкеты в этом сообщении.\n\n"
                 "Нужны строки вида «Ребёнок: …» / «Взрослый: …» — перешлите "
                 "сообщения бота целиком.",
                 CANCEL_MENU)
            return
        existing = {_norm_name(s["name"]) for s in core.list_students()}
        fresh = [a for a in anketas if _norm_name(a["name"]) not in existing]
        skipped = len(anketas) - len(fresh)
        if not fresh:
            _state.pop(user_id, None)
            send(peer_id, f"Все эти ученики ({skipped}) уже есть в базе — "
                          f"добавлять нечего.", ADMIN_MENU)
            return
        st["data"]["anketas"] = fresh
        st["step"] = "confirm_anketas"
        lines = []
        for a in fresh:
            who = "ребёнок" if a["category"] == "child" else "взрослый"
            extra = f", родитель: {a['parent_name']}" if a.get("parent_name") else ""
            lines.append(f"• {a['name']} ({who}{extra})")
        msg_text = f"Нашёл анкет: {len(fresh)}\n\n" + "\n".join(lines)
        if skipped:
            msg_text += f"\n\n(пропущено, уже в базе: {skipped})"
        msg_text += "\n\nВосстановить их?"
        send(peer_id, msg_text, {
            "one_time": False, "inline": False,
            "buttons": [[
                _btn("✅ Да, восстановить", "restore_ok", color="positive"),
                _btn(BTN_CANCEL, "restore_cancel", color="negative"),
            ]],
        })
        return

    if step == "confirm_anketas":
        send(peer_id, "Пожалуйста, подтвердите кнопкой ниже 👇")
        return
...
def handle_group_join(obj: dict):
    ...
    if core.is_paused(user_id, "vk"):
        return
    result = send(user_id, WELCOME_TEXT, WELCOME_MENU)
    if result is not None:
        log.info("Подписчик %s: отправлено приветствие", user_id)
    else:
        log.info(
            "Подписчик %s: не удалось отправить приветствие сразу "
            "(скорее всего, запрещены сообщения от сообществ) — покажется, "
            "когда он сам напишет боту",
            user_id,
        )
...
def run():
    core.init_db()
    # Где физически лежит база — пишем в лог при каждом старте. Если она
    # окажется во временной папке, данные исчезнут при следующей пересборке,
    # и лучше узнать об этом сразу, а не после потери учеников.
    if os.environ.get("DATA_DIR", "").strip():
        log.info("База данных: %s (постоянный каталог)", core.DB_PATH)
    else:
        log.warning(
            "ВНИМАНИЕ: переменная DATA_DIR не задана, база лежит рядом с кодом (%s). "
            "На хостинге это означает, что все ученики пропадут при следующей "
            "пересборке. Задайте DATA_DIR в переменных окружения.",
            core.DB_PATH,
        )
    log.info("Учеников в базе: %s", len(core.list_students()))
